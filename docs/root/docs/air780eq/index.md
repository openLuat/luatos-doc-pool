...
- [合宙Air780EQ模组产品手册](https://docs.openluat.com/air780eq/product/)
...
1. 从用户实际使用的角度，解答大家对Air780EQ最关心的一些问题；
...
3. Air780EQ软件上既支持传统的AT指令，也支持合宙基于Lua脚本开发的嵌入操作系统LuatOS；
4. Air780E硬件上支持的外设管脚相对较少，仅支持USB、UART和部分GPIO，相对Air700ECQ/Air700EAQ/Air700EMQ/Air780EP/Air780EPS都较少一些；
5. Air780EQ支持丰富的网络协议，比如TCP/UDP、TCP-SSL/TCP-TLS、MQTT、HTTP、WEBSOCKET、NTP等；
6. Air780EQ不支持TTS语音播放，也不支持VoLTE语音通话，需要支持这两个功能的应用推荐选择合宙Air724UG；
7. Air780EQ功耗表现在合宙现有模组中排名中等，低功耗模式下0.5mA，低功耗表现最好的模组型号是合宙Air780EPS；

### Air780EQ实网功耗数据

| Air780EQ                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | 常规模式                    | 低功耗模式                                   | PSM+模式                                                  |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------- | -------------------------------------------- | --------------------------------------------------------- |
...
1)Air780EQ，供电电压3.8V，移动网络，频段B3，RSRP值-88附近，DRX 2.56秒，心跳间隔5分钟，心跳数据100Byte，TCP协议，合宙服务器，回环测试；

2)Air780EQ，同等环境下，低功耗模式，DRX 1.28秒时，平均电流0.9mA，DRX 0.64秒时，平均电流1mA；

3)Air780EQ，同等环境下，常规模式，DRX 1.28秒时，平均电流7mA，DRX 0.64秒时，平均电流7.2mA；

4)DRX，Discontinuous Reception，非连续接收，可简单理解为模块与基站之间保持心跳的间隔，一般为0.64秒/1.28秒/2.56秒，需要注意的是，DRX由基站根据网络实际情况而定，模组无法自行控制；

5)Air780EQ功耗表现在合宙现有模组中排名中等，低功耗模式下平均电流0.5mA，低功耗表现最好的模组型号是合宙Air780EPS，请根据需要灵活选择； 
...
1. **Air780EQ支持C-SDK开发吗？**

   Air780EQ不支持C-SDK开发，推荐您使用LuatOS开发方式；
...
2. **Air780EQ支持FOTA功能吗？**
   ...
   Air780EQ支持差分升级，无论AT软件，还是LuatOS软件，都可以通过合宙IoT后台进行FOTA升级；
...
5. **Air780ER与Air780EQ有什么关系？**

   Air780ER与Air780EQ封装尺寸完全相同；

   Air780ER与Air780EQ在产品功能定义上不同，Air780EQ主打AT和LuatOS，Air780ER主打USB上网功能；

   配合Windows/Linux/Android操作系统，结合Air780ER的RNDIS/PPP功能，可快速实现USB上网功能；

6. **Air780EQ可以支持海外市场吗？**

   Air780EQ的频段(FDD:B1/3/5/8;TDD:B34/38/39/40/41)可以支持中国移动、电信、联通三大运营商；

   印度与中国的4G频段相同，理论上Air780EQ也可以支持印度，但印度运营商众多，建议出货前务必场测；

   东南亚诸国与中国的4G频段接近，但部分国家会略有不同，多集中在B7和B28的差异，贸然出货不排除在部份区域由于基站频段与Air780EQ频段的不匹配而导致无法通信；
