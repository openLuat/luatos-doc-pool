...
1. 从用户的角度，解答大家对Air780EX这款模组最关心的问题；
...
![](https://e3zt58hesn.feishu.cn/space/api/box/stream/download/asynccode/?code=OTA4NzMyZTY4NmY5NzMxYzRkYjE0M2M5ZTEyODEwMWJfNGpNamVMU0ozU1dURUtrVXhPVnRHR2IzN3haTGJ3NTBfVG9rZW46U1M4OWJXazdZb2xnWjZ4MGRlV2NrUVpvbmpmXzE3MjgyODg5NTc6MTcyODI5MjU1N19WNA)

管脚透视图

![](https://e3zt58hesn.feishu.cn/space/api/box/stream/download/asynccode/?code=NGEzYjA2Y2M3ZDAwMTY1NzY2ZmJkYzk4MjhlZmZkNWVfWFY4ZTVZRFNoUWFHb1BVZm5kcEZtNWczN0ZTamRxY1FfVG9rZW46S25qVGJYSnFIb2FxNkF4N0JzZWNyaG91bjlmXzE3MjgyODg5NTc6MTcyODI5MjU1N19WNA)
...
3. Air780EX软件上既支持传统的AT指令，也支持合宙基于Lua脚本开发的嵌入操作系统LuatOS；
4. Air780EX硬件上支持丰富的外设管脚，比如USB、UART、I2C、PWM、GPIO等，相对Air780E较少一些；
5. Air780EX支持丰富的网络协议，比如TCP/UDP、TCP-SSL/TCP-TLS、MQTT、HTTP、WEBSOCKET、NTP等；
6. Air780EX不支持TTS语音播放，也不支持VoLTE语音通话，需要支持这两个功能的应用推荐选择合宙Air724UG；
7. Air780EX功耗表现在合宙现有模组中排名中等，低功耗模式下低于0.45mA，低功耗表现最好的模组型号是合宙Air780EPS；

### Air780EX实网功耗数据

| Air780EX                         | 常规模式                    | 低功耗模式                                   | PSM+模式                                                  |
| -------------------------------- | --------------------------- | -------------------------------------------- | --------------------------------------------------------- |
...
1)Air780EX，供电电压3.8V，移动网络，频段B3，RSRP值-88附近，DRX 2.56秒，心跳间隔5分钟，心跳数据100Byte，TCP协议，合宙服务器，回环测试；

2)Air780EX，同等环境下，低功耗模式，DRX 1.28秒时，平均电流0.6mA，DRX 0.64秒时，平均电流0.95mA；

3)Air780EX，同等环境下，常规模式，DRX 1.28秒时，平均电流4.8mA，DRX 0.64秒时，平均电流4.9mA；

4)DRX，Discontinuous Reception，非连续接收，可简单理解为模块与基站之间保持心跳的间隔，一般为0.64秒/1.28秒/2.56秒，需要注意的是，DRX由基站根据网络实际情况而定，模组无法自行控制；

5)Air780EX功耗表现在合宙现有模组中排名中等，低功耗模式下低于0.5mA，低功耗表现最好的模组型号是合宙Air780EPS；
...
1. **Air780EX支持C-SDK开发吗？**

   Air780EX不支持C-SDK开发，推荐您使用LuatOS开发方式；
...
2. **Air780EX支持FOTA功能吗？**
   ...
   Air780EX支持差分升级，无论AT软件，还是LuatOS软件，都可以通过合宙IoT后台进行FOTA升级；

3. **Air780EX支持数据透传吗？**

   Air780EX支持数据透传功能；

   无论AT指令，还是LuatOS，Air780EX都可以方便的实现数据透传功能；
...
5. **Air780EX可以支持海外市场吗？**
   ...
   Air780EX的频段(FDD:B1/3/5/8;TDD:B34/38/39/40/41)可以支持中国移动、电信、联通三大运营商；

   印度与中国的4G频段相同，理论上Air780EX也可以支持印度，但印度运营商众多，建议出货前务必场测；

   东南亚诸国与中国的4G频段接近，但部分国家会略有不同，多集中在B7和B28的差异，贸然出货不排除在部份区域由于基站频段与Air780EX频段的不匹配而导致无法通信；
